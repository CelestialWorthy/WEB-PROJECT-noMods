...
const getTutorsBySubject = async (request, response) => {
  try {
    const subject = request.params.subject;
    const tutors = await sequelize.query("SELECT * FROM tutors WHERE subject = ?", {
      type: QueryTypes.SELECT,
    ...
    response
      .status(500)
      .json({ msg: "Error on getting tutors by subject", error: error.message });
  }
};


module.exports={getTutors,postTutor,getTutorsBySubject};